...
The conditions are presented in a sequence of `WHEN... THEN` clauses, so that the `CASE` expression as a whole is assigned the value of the first condition that is true.
...
Each `WHEN... THEN` clause presents its own expression (value).

...
Since global accumulators are shared in a mutually exclusive manner among processes, they behave very differently than a non-accumulator variable (see the xref:declaration-and-assignment-statements.adoc#_declaration-statements[Declaration Statements] section for more details) in an `ACCUM` clause.
...
SetAccum also can be used with the three canonical set operators: `UNION`, `INTERSECT`, and `MINUS` (see the xref:operators-and-expressions.adoc#_setbag-expression-and-operators[Set/Bag Expression and Operators] section for more details).

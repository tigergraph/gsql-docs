...
type := baseType | containerType | tupleType | accumType 
...
[NOTE]
====
When referring to a `FILE` object, this documentation capitalizes the word `FILE` to distinguish it from ordinary files.
====

A `FILE` object can also point to cloud storage instead of a local file, e.g. `FILE f ("s3://bucket-name/queryoutput/output.csv");`.
See xref:declaration-and-assignment-statements.adoc#_file_objects[FILE Object Declaration] for more details.
...
For how to declare the parameters in a query, see xref:querying:create-and-install-queries.adoc#_create_query_syntax_elements[CREATE QUERY: parameterList].

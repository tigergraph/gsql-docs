...
The first seven types (`INT`, `UINT`, `FLOAT`, `DOUBLE`, `BOOL`, `STRING`, and `DATETIME`) are the same ones mentioned in xref:ddl-and-loading:attribute-data-types.adoc#_primitive_types[Attribute Data Types] of the GSQL Reference.

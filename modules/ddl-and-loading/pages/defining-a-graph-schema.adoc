...
. Using xref:_primary_id[`PRIMARY_ID`] to define the ID.
...
. Using xref:_primary_key[`PRIMARY KEY`] to designate an attribute as the ID.
. Using xref:_composite_key_using_primary_key[`PRIMARY KEY`] with a parenthesized list of attributes, to create a compound key.
...
Its syntax for the `primary_id_name_type` term is as follows:

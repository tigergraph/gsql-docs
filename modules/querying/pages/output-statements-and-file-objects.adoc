...
If the query response is larger than 2GB, it is recommended to use <<_printing_csv_to_a_file_object>>. TigerGraph currently does not support pagination; returning a large query response in a single response via the GSQL terminal or RESTPP may lead to excessive memory usage and make the response difficult to parse efficiently.
====
...
xref:ddl-and-loading:null-value.adoc[NULL values] can be printed using the `PRINT` statement and are represented as `null` in JSON and CSV output [v4.3+].


.Printing nullable attribute values
[source,gsql]
----
// Company(PRIMARY_ID client_id STRING, id STRING, country STRING NULLABLE)
// Assume NULL is loaded for 'country' on some vertices.

CREATE QUERY print_null_company() FOR GRAPH Work_Net {
    C = {Company.*};
    companies = SELECT t FROM C:t;
    PRINT companies[companies.country]; // print null value
}
----

.JSON output with null value
[source,json]
----
{
  "version": {
...
For details on writing output to S3 or S3-compatible storage, see xref:write-output-to-cloud.adoc[Write Output to Cloud].
[source,gsql]
----
...
As of 4.3, users can declare a FILE object with a linux octal-encoded file permission code to specify file permissions.
See xref:querying:declaration-and-assignment-statements.adoc#_file_objects[Declaration and Assignment Statements] page for details.
...
.Results from Query get_US_worker_interests
...
.File contents produced by get_US_worker_interests example

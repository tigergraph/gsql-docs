...
[NOTE]
====
REST endpoints return results in JSON format. JSON data are used for various purposes. However, JSON data can't be read easily from JSON files by using bash scripts the way other files can. The *jq* tool is used to solve this problem.
...
*Example*. Find all friendship edges whose source vertex's primary_id is "Tom". In other words, find all of Tom's friendships.

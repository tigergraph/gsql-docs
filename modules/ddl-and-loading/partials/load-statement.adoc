...
Not supported in a xref:tigergraph-server:data-loading:kafka-loader/index.adoc[Kafka loading job].
...
TigerGraph can load data from Parquet files if they are stored in AWS S3 buckets.
For more details on how to set up S3 data sources and loading jobs, read the xref:3.7@tigergraph-server:data-loading:s3-loader-user-guide.adoc[AWS S3 Loader User Guide].

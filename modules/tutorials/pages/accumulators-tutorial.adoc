...
In addition to provide the classic xref:tutorials:pattern-matching/index.adoc[pattern match] syntax, which is easy to master, GSQL supports powerful run-time vertex attributes (a.k.a local accumulators) and  global state variables (a.k.a global accumulators).

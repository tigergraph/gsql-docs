...
Use the following session parameters in GSQL:
[source,gsql]
----
...
* `set query_timeout=7200000;` sets the query timeout (this is optional but can be adjusted based on your query's expected duration).
* `set s3_aws_access_key_id = <AWS_KEY_ID>;` specifies your AWS access key ID.
* `set s3_aws_secret_access_key = <AWS_ACCESS_KEY>;` specifies your AWS secret access key.

These session parameters should be set within the GSQL Editor to enable read/write access to the specified S3 bucket for query results.

For more information on exporting query data to S3, refer to this xref:graph-development:advanced-features:write2-s3.adoc[detailed guide].

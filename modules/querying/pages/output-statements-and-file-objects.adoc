...
* Expanded forms like `PRINT A[A.*]`; or `PRINT A.* TO_CSV f`; are not supported. The system will return an error with a hint to use `PRINT A`; or `PRINT A TO_CSV f`; instead.

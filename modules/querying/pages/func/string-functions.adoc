...
Returns a character string containing the https://en.wikipedia.org/wiki/Soundex#:~:text=Soundex is a phonetic algorithm,despite minor differences in spelling.&text=Improvements to Soundex are the basis for many modern phonetic algorithms.[Soundex] code of `str`. This function lets you compare words that are spelled differently, but sound alike in English.

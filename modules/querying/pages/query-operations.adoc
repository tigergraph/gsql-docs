...
A query description provides additional information about a query or its parameters. 
...
GSQL > SHOW DESCRIPTION OF QUERY_PARAM query1.param2
GSQL > SHOW DESCRIPTION OF QUERY_PARAM query2.*
----

* You cannot combine parameters from different queries in one command. Each query or parameter must be specified individually.
...
GSQL > DROP DESCRIPTION OF QUERY ALL ON GRAPH poc_graph
...
GSQL > DROP DESCRIPTION OF QUERY_PARAM * ON GRAPH poc_graph
GSQL > DROP DESCRIPTION OF QUERY_PARAM ALL ON GRAPH poc_graph
----

...
Schema changes are crucial for maintaining and improving your graph's structure. However, they must be handled carefully to avoid disrupting ongoing operations.
By following best practices, you can minimize the risk of failures and ensure smooth execution.

=== Dos and Don'ts During Schema Changes

*Do's:*

* *Do ensure no active queries or loading jobs.* Only perform schema changes after aborting any ongoing jobs that interact with the components being modified.
* *Do perform schema changes during periods of minimal graph activity.* A "low-traffic" period refers to a time when there are no active queries or data loading jobs running. This reduces the risk of conflicts and failures during schema changes.

*Don'ts:*

* *Don’t run queries or loading jobs during schema changes.* These jobs may fail if they interact with modified parts of the schema.
...
Defining periods of low and high traffic is crucial to optimizing schema changes and reducing failure risks.

*Low Traffic:*

* Schema changes should be scheduled when there are minimal active operations (queries or data loads) to avoid conflicts.

*High Traffic:*

* Avoid schema changes during periods of high query or data load activity to prevent failure or inconsistent behavior.

=== Risks of Running Jobs During Schema Changes

Running queries or loading jobs while modifying the schema can lead to data inconsistency or errors. For instance, if a schema change modifies a vertex type and a query or loading job is still active, it will likely fail.

*Best Practice:* Always abort active queries and loading jobs before performing a schema change.

=== Verifying Schema Integrity After Changes

Once the schema change is complete, verify data and schema consistency:

*Verifying Data Consistency:*

* *Backup your data* before performing a schema change to protect against potential data loss or corruption.
* Run the `gstatusgraph` command to check that the data size remains consistent with what was recorded before the schema change. This helps ensure that no data loss or corruption has occurred during the schema update.
* While schema changes are generally safe, unexpected issues may arise. Taking a backup ensures you can recover from any unforeseen problems that could result in data inconsistencies.

*Verifying Schema Consistency:*

* To ensure that the schema change has been applied correctly and that no unintended changes have occurred, run the following script:

=== Impact on Queries and Loading Jobs

Schema changes will invalidate queries and loading jobs that depend on modified schema components.
Specifically, a loading job becomes invalid if it references a vertex or edge that has been DROPPED or ALTERED. Similarly, a query becomes invalid if it refers to a DROPPED vertex, edge, or attribute. Invalid loading jobs are DROPPED, and invalid queries are uninstalled.
After the schema update, users must create and install new loading and query jobs based on the updated schema.

*What to Do:*

* After schema changes, reinstall and redefine queries and loading jobs based on the new schema.
* Ensure that the updated queries and jobs align with the modified schema and are properly tested to ensure they work as expected.

[IMPORTANT]
====
Perform schema changes only when no active queries or loading jobs are running to avoid data consistency and application errors.
Always abort ongoing jobs before initiating the schema change. 

Additionally, perform schema changes during low traffic periods to minimize failure risks.
====

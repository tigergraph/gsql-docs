...
The `SHOW DESCRIPTION` command displays the description of a query or a query parameter.
...
* For the `SHOW DESCRIPTION` command, you can display the descriptions of individual queries or query parameters. Each query or parameter must be specified separately, and you cannot combine them in a single request.
* The wildcard `*` can be used to show descriptions of all queries or all parameters for a specific query. 
* `SHOW DESCRIPTION` also supports a wider range of regular expressions, allowing for more advanced querying when searching for descriptions.

=== Drop description

The `DROP DESCRIPTION` command is used to remove descriptions of queries or query parameters. 
...
* For the `DROP DESCRIPTION` command, you can specify multiple queries or parameters in a single command using a comma-separated list. However, the `SHOW DESCRIPTION` command does not support this feature.

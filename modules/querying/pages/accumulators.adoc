...
* Edge accumulators can be declared anywhere in the query except for in a `FOREACH` loop or `WHILE` loop.
...
Edge accumulators are mutable state variables that are attached to each edge in the graph for the duration of the query's lifetime.
They act as run-time attributes of a edge.
They are shared, mutually exclusively, among all query processes.

Edge accumulators can be set to a value with the `=` operator.
Additionally, the operator `+=` can be used to update the state of the accumulator; the function of `+=` depends on the accumulator type.
Edge accumulators can only be accessed or updated (via `=` or `+=`) in an `ACCUM/WHERE` clauses within a `SELECT` statement.
...
A statement such as ``e1.@min += 1``will only impact `e1` 's `@count` and not the `@count` for other edges.

.Edge Accumulators in a Query
[source,gsql]
----
CREATE QUERY test() {
    SumAccum<INT> EDGE @count;

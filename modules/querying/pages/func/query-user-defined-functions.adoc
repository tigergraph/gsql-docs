...
For more details, see xref:tigergraph-server:manage:security/index.adoc#_udf_file_scanning[Security - UDF file scanning].
...
Use the xref:manage:management-commands.adoc#_gadmin_config_set[`gadmin config set`] command to configure the aforementioned parameters to connect GSQL to the GitHub repository hosting your files.
...
This command is securely managed by xref:manage:management-commands.adoc[ the management tool `gadmin`] and is disabled by default, requiring a Tigergraph Linux user with gadmin access to enable it.

...
See the xref:querying:run-manage-queries.adoc#_parameter_list[RUN QUERY parameter list] for details.
...
you can also authenticate using AWS role assumption. Set the `s3_assume_role` session parameter to the role ARN [4.3+].
...
For how to declare the parameters in a query, see xref:querying:run-manage-queries.adoc#_parameters[CREATE QUERY: parameterList].

For how to run a query, see

* xref:querying:run-manage-queries.adoc#_query_parameters[Parameters when running a query in the GSQL CLI].

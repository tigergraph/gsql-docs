...
* If the input cannot be converted to a boolean, it returns `null`.
* If the input is `null`, it also returns `null`.
...
* If the input is already a floating-point number, it is returned unchanged.
* If the input cannot be converted, `null` is returned.
* If the input is `null`, it also returns `null`.

...
* Provide an absolute path or relative path for either a file or a folder on the machine where the job is run.
...
* `"any:" path`: If the path is prefixed with `any:` , then the loading job will attempt to run on every machine in the cluster which has a RESTPP component, and each machine will look locally for data at the specified path.
...
|int
|Returns day (1-31) from timestamp. Refer

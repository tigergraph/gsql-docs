...
For example, the following query definition sets the default value of `d1` to `2019-01-19 00:00:00`.

[.wrap,gsql]
----
CREATE QUERY print_default(DATETIME d1 = to_datetime("2019-01-19 00:00:00")) {
    PRINT d1;
}
----
...
Note that there is a difference between a typed and a generic `VERTEX` parameter.
Queries that could take any vertex type as input usually require generic `VERTEX` parameters so that the user specifies the ID and the type.

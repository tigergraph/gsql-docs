...
* **Collection attributes (`LIST`, `SET`, `MAP`)** — the new value(s)  are appended to the old value. This behavior is similar to xref:gsql-ref:ddl-and-loading:creating-a-loading-job.adoc#_loading_a_list_or_set_attribute[cumulative loading for collection attributes].
...
insertVertexStmt := INSERT INTO vertexType
                 ["(" PRIMARY_ID ["," attrName]* ")"]

...
GSQL's OpenCypher mode ("GSQL OpenCypher") is strongly type and **schema-first**, while OpenCypher is **schema-optional**. This fundamental difference impacts how OpenCypher queries behave in each system.
...
* In GSQL OpenCypher mode, queries must align with the predefined schema.

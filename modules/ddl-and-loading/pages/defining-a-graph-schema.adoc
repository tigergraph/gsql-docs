...
You can also add more endpoint vertex pairs to an edge type
...
Between two vertices, only one instance of a certain edge type is allowed to exist if the edge definition does not include a discriminator.
...
For example, the following `CREATE EDGE` statement specifies that the edge can be uniquely identified by the combination of their `class_year` and `class_month` attributes:

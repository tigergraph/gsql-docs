...
In GSQL V3, starting with version 4.1.3, we support the use of the equality operator `=` and the inequality operator `<>` in addition to the existing `==` and `!=` operators.
...
* xref:run-manage-queries.adoc#_interpret_query[Interpreted mode] is not supported.

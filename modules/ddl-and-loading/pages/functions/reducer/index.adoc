...
* For `LIST`, `SET`, and `MAP` containers, the `input_expr` type is one of the xref:ddl-and-loading:attribute-data-types.adoc#_collection_types[allowed element types] for these containers.

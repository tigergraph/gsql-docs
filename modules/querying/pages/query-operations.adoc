...
The `-PROFILE` option allows you to activate xref:querying:query-optimizer/query-profiling.adoc#_gsql[query profiling] when running a query in GSQL. Currently, the only supported value for profiling is `BASIC`, which enables basic query profiling to view and analyze execution details.

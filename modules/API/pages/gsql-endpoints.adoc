...
| `gsql`
| yes
| Indicates whether to use the GSQL command for creation. Must be set to true. The request body should contain the GSQL command within the JSON object.
|===


'''     

=== create vertex (add global vertices to local graph)

`POST /gsql/v1/schema/vertices`

This endpoint is used to add existing global vertices to graph 
...
| `gsql`
| yes
| Indicates whether to use the GSQL command for adding vertices. Must be set to false. The request body should be in JSON format.

| `graph`
| yes
| Specifies the graph to which the global vertices should be added.
|===
...
[source,bash]
----
curl -X POST -H "content-type: application/json" "http://localhost:14240/gsql/v1/schema/vertices?gsql=false" -d '{
    "createVertices": [
        {
            "Config": {
...
| `gsql`
| yes
| Indicates whether to use the GSQL command for creating vertices. Must be set to false. The request body should be in JSON format.
|===
...
| `gsql`
| yes
| Indicates whether to use the GSQL command for creating edges. Must be set to true. The request body should contain the GSQL command within the JSON object.
|===
...
| `gsql`
| yes
| Indicates whether to use the GSQL command for creating edges. Must be set to false. The request body should use the JSON object.
...
| `graph`
| yes
| Specifies the name of the graph for which the schema details are to be shown.
|===
...
| `gsql`
| yes
| Indicates whether to use a GSQL command for graph creation. Set to false to use JSON format.
...
| `cascade`
| no
|  If set to `true`, it will automatically drop the queries and loading jobs associated with this graph. If set to `false`, the operation will fail if there are any existing queries or loading jobs related to the graph.
|===
...
| `type`
| yes
| Specifies the type of schema change job to create. Use `global` for creating a global schema change job and `local` for creating a local schema change job.
|===
...
| `gsql`
| yes
| Indicates whether to use the GSQL command for creation. Must be set to false. The request body should use the JSON object.

| `type`
| yes
| Specifies the type of schema change job to create. Use `global` means creating a global schema change job.
|===
...
| `graph`
| yes
| The graph for which the schema change job is to be created.

| `type`
| yes
| Specifies the type of schema change job to create. Use `local` means creating a local schema change job.
|===
...
| `graph`
| yes
| the graph who the local schema change runs on, If not provided, means running global schema change job.
|===
...
| `graph`
| no
| The graph who the schema change job runs on. If not provided, means run the global schema change job.
|===
...
| `isPause`
| no
| true means pause or abort the loading job, false means run the loading job.
|===
...
| `path`
| yes
| the path where the data located used to restore the data

| `reinstall`
| no
| `true`, force to install all queries, otherwise will not.

| `restore`
| no
| `true`, will clean replicationManager unwritten replicas. otherwise will not.

| `keepUser`
| no
| `true`, will retain users and permissions when restoring from a backup, ignoring the users in the backup. otherwise use the user in backup data.
|===
...
[source,bash]
----
curl -H 'Content-Type: text/plain' -X GET 'localhost:8123/gsql/v1/saml/meta' 
----
--
Response::
+
...
[source.wrap,console]
----
<?xml version="1.0"?><md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata" validUntil="2024-08-03T08:24:45Z" cacheDuration="PT604800S" entityID="http://127.0.0.1:14240/gsql/v1/saml/meta" ID="ONELOGIN_5296b4b6-3dec-41b3-9f37-531db41aba25"><md:SPSSODescriptor AuthnRequestsSigned="true" WantAssertionsSigned="true" protocolSupportEnumeration="urn:oasis:names:tc:SAML:2.0:protocol"><md:SingleLogoutService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect" Location="http://127.0.0.1:14240/logout"/><md:NameIDFormat>urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified</md:NameIDFormat><md:AssertionConsumerService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="http://127.0.0.1:14240/api/auth/saml/acs" index="1"/></md:SPSSODescriptor><md:Organization><md:OrganizationName xml:lang="en">TigerGraph Inc.</md:OrganizationName><md:OrganizationDisplayName xml:lang="en">TigerGraph Inc.</md:OrganizationDisplayName><md:OrganizationURL xml:lang="en">https://www.tigergraph.com/</md:OrganizationURL></md:Organization><md:ContactPerson contactType="technical"><md:GivenName>TigerGraph Support Team</md:GivenName><md:EmailAddress>support@tigergraph.com</md:EmailAddress></md:ContactPerson><md:ContactPerson contactType="support"><md:GivenName>TigerGraph Support Team</md:GivenName><md:EmailAddress>support@tigergraph.com</md:EmailAddress></md:ContactPerson></md:EntityDescriptor>
----
--
====
...
`POST /gsql/v1/internal/replication?newSeqId=<SeqId>`
...
----
{
    ...
    "message": "",
    "results": {
        "SAMLRequest": "",
        "SigAlg": "http://www.w3.org/2001/04/xmldsig-more#rsa-sha256",
        "Signature": "",
        "ssoBinding": "POST",
        "ssoUrl": "https://dev-71097685.okta.com/app/dev-71097685_tigergraphqe02_1/exkqeyfvzqPHcJVKp5d6/sso/saml"
    }
}
...
[source,bash]
----
curl -H 'Content-Type: application/json' -X GET 'localhost:8123/gsql/v1/oidc/authnrequest' 
----
--
Response::
+
...
| `clear`
| no
| true means clear out the current block list, default value is false.
|===
...
This endpoint is used to retrieve a group by id using scim format or gsql format.

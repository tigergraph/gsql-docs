...
1. To log in using the token, type the following command in the terminal:
[source,gsql]
----
gsql -t <valid_token>
----

2. Once you’re logged in, run any GSQL command. For example, to list all vertices of type `user`, run:
[source,gsql]
----
GSQL > SELECT * FROM user;
----

3. After you're done, exit the GSQL shell by typing:
[source,gsql]
----
GSQL > EXIT
----

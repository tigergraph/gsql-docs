...
After we've tackled single hops, we'll see how to add repetition to make variable length patterns and how to connect single hops to form bigger patterns.

In GSQL syntax V2, we use the punctuation `-( )-` to denote a 1-hop pattern. This can also be written as `- ( ) -`. The spaces do not affect the output.
...
Arrowheads are used to indicate direction, e.g.,  `>` or `<`.

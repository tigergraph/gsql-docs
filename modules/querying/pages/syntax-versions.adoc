...
|Directionality applies to the entire edge pattern. 
[source]
----
Symbols include `<-` (left), `->` (right), `~` (undirected).
----
...
|V2 supports explicit repetition for multi-hop patterns. For example:
[source]
----
-(FRIENDS*1..3)->
----
|V3 simplifies multi-hop patterns by using `{m,n}` GQL-style quantifiers. For example:
[source]
----
-[FRIENDS]->{1,3}
----
...
-[FRIENDS]->{1,3}
----
* In V3, quantification is expressed using {m,n} syntax for variable-length edge patterns, which simplifies the syntax and provides a more consistent approach.

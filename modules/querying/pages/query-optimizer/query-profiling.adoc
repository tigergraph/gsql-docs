...
This page provides an overview of query profiling in TigerGraph, a feature introduced in version 4.2. 
...
Activate profiling by setting a profiling mode. Currently, only one profiling mode, `BASIC`, is available.
...
*Option 1*:
To activate profiling for a single query and view the profile information in real-time, add the `PROFILE: BASIC` header to your RESTPP request:
...
*Option 1*:
In GSQL, you can activate profiling for a single query and view the profile information during execution using the following command:

[source, gsql]
----
RUN QUERY -PROFILE BASIC qtest()
----

*Option 2*:
To run a query asynchronously in GSQL with profiling enabled, use the `-async` flag:

[source, gsql]
----
RUN QUERY -async <queryName>
----

Make sure to replace `<queryName>` with the name of the query you are running.

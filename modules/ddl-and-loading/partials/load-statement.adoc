...
|`USER_DEFINED_HEADER`
|Specifies the name of a header variable defined by a preceding DEFINE HEADER statement. The loader uses the user-defined column names instead of the column names in the input file. For an example, see xref:data-loading:load-local-files.adoc#user-defined-header-example[Example: Using USER_DEFINED_HEADER].

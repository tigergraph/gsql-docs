...
Use the underscore `\_` or the keyword `ANY` as a wildcard meaning "any edge type."
Arrowheads are used to indicate direction, e.g.,  `>` or `<`.
An "any edge type" wildcard without the arrowheads will be treated as undirected.

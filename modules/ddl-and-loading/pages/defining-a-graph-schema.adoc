...
The xref:#_with_primary_id_as_attribute[primary_id_as_attribute] option was described with PRIMARY ID.
...
For more about roles and privileges, see xref:{page-component-version}@tigergraph-server:user-access:access-control-model.adoc[].
...
`SHOW PACKAGE` lists the packages that are available, such as the packaged template queries. See xref:graph-ml:using-an-algorithm:index.adoc#_installrun_as_packaged_template_queries[Using an Algorithm - Packaged template queries] for more information.

...
tableName := name
----
<1> If a SQL-like `SELECT` statement has an xref:{page-component-version}@gsql-ref:querying:select-statement/README.adoc#_order_by_clause[`ORDER BY` clause], it must also have a `LIMIT` clause.

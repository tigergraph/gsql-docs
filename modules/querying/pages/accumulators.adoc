...
[NOTE]
====
TigerGraph supports edge accumulators starting in 3.10.0, but only for Single GPR mode query.
====
...
[NOTE]
====
Currently, we only support edge attached accumulators in single GPR mode! Please install the query with 'set single_gpr = true' or '-single' flag to use this feature.
====
...
| `PercentileContAccum`
| `0`
| `PercentileContAccum` returns the percentile of the given value over a group using linear interpolation.
...
PercentileContAccum returns the percentile of the given value over a group using linear interpolation.
PercentileContAccum’s default value is `0`, and is of double type.

It is similar to AvgAccum, and does not need to set the data type.

.PercentileContAccum() has two way of initialization:
[source, gsql]
----
PercentileContAccum @@ps; //the percent is 0.0
PercentileContAccum<0.5> @@ps; //the percent is 0.5
----

.PercentileContAccum also has the following functions available:
* `setpercent(double percent)`: to set the percent, the default value of percent is `0`.
* `getpercent()`: to check current value of a percent.
* `clear()`: to clear the inner vector that holds all the data, and reset default value of the accumulator to `0`.
This will not change the percent.
...
+
--
.PercentileContAccum Example
[source,gsql]
----
CREATE OR REPLACE QUERY test1 () {
    TYPEDEF PercentileContAccum<0.4> perc; // Usage with TYPEDEF
    GroupByAccum<int __dummyKey, perc pc> @@T;
    VS_XXX = SELECT p
             FROM   Person:p
...
CREATE OR REPLACE QUERY test2 () {
    PercentileContAccum<0.9> @@pa, @@ps;
    PercentileContAccum @@ps_post;
    // Default value of PercentileContAccum
    print @@pa, @@pa.getpercent();
    ...
    VS_XXX = SELECT p
             ...
             ACCUM
                  @@ps += datetime_to_epoch(p.creationDate)
             POST-ACCUM
                  @@ps_post += p.id;
    PRINT @@ps, @@ps.getpercent(), @@ps_post, @@ps_post.getpercent();

    @@ps.setpercent(0.6);
    @@ps_post.setpercent(0.6);
    PRINT @@ps, @@ps.getpercent(), @@ps_post, @@ps_post.getpercent();

    // clear function will clear data, but not percent
    @@ps_post.clear();
    PRINT @@ps_post, @@ps_post.getpercent();
}

// local accumulator
CREATE OR REPLACE QUERY test3 (DATETIME date_1, DATETIME date_2) {
     PercentileContAccum<0.3> @lperc_1;
     PercentileContAccum @lperc_2;
     PercentileContAccum @@gperc;
     @@gperc.setpercent(0.6);

     S = SELECT s FROM tupPerson:s -(<TUP_MEMBER:e)- _:t
          WHERE e.creationDate > date_1 AND e.creationDate < date_2
          ACCUM
               s.@lperc_2 += s.id2,
               s.@lperc_2 += s.id2 + 1000,
               s.@lperc_2 += s.id2 + 3000
          POST-ACCUM(s)
               s.@lperc_2.setpercent(0.4),
               @@gperc += s.@lperc_2;

     PRINT S [S.@lperc_2, S.@lperc_2.getpercent(), S.@lperc_1.getpercent()], @@gperc, @@gperc.getpercent();
}

CREATE OR REPLACE QUERY test4 (DATETIME date_1, DATETIME date_2) {
     TYPEDEF PercentileContAccum<0.3> perc_1;
     TYPEDEF PercentileContAccum perc_2;
     perc_1 @lperc_1;
     perc_2 @lperc_2;
     PercentileContAccum @@gperc;
     @@gperc.setpercent(0.6);

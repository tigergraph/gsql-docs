...
a. To log in using the token, type the following command in the terminal:
[source,gsql]
----
gsql -t <valid_token>
----

b. Once you’re logged in, run any GSQL command. For example, to list all vertices of type `user`, run:
[source,gsql]
----
GSQL > SELECT * FROM user;
----

c. After you're done, exit the GSQL shell by typing:
[source,gsql]
----
GSQL > EXIT
----

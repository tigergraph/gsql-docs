...
The xref:{page-component-version}@tigergraph-server:api:gsql-endpoints.adoc#_example_42[`maxPercentError`] key for running a loading job via the REST API has the same meaning as the `-max_percent_error` option in GSQL.
...
The xref:{page-component-version}@tigergraph-server:api:gsql-endpoints.adoc#_example_42[`maxNumError`] key for running a loading job via the REST API has the same meaning as the `-max_error` option in GSQL.

...
The path must start with `s3://`, followed by the bucket name and the S3 path, for example:

`s3://bucket-name/queryoutput/output.csv`

During query execution, content is uploaded to the S3 bucket.

If an S3 object with the same path already exists, it is overwritten.
An S3 object cannot be appended.

==== Set S3 connection credentials

The S3 credentials and region can be set as GSQL session parameters, so they persist for a user for a full session.
[source,gsql]
----
...
`\_GPE_{PartitionId}_{ReplicaId}`

For distributed queries, additional suffixes such as `_coordinator` and `_worker` are added to distinguish roles.
...
For local files, multiple queries can append to the same `FILE`.

For S3 `FILE` objects, content cannot be appended.
If the S3 object already exists, it is overwritten.
====

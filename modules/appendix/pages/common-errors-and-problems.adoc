...
=== Exact comparison of floating point numbers is not appropriate
...
=== Adding floating point values at different scales is problematic
...
What is `A + B`?
In pure math, the answer is `1234567.1234567`.
But that requires 14 digits of precision, while `FLOAT` offers only about 7 digits.
This problem can occur inadvertently when adding a large set of numbers.
The individual numbers may be about the same scale, but the intermediate sums are of a larger scale.

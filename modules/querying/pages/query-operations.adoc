...
=== log output
`INSTALL QUERY` displays the following log details:

* *Installation summary*: A summary line prints when installation
  finishes, showing the total count of succeeded, skipped, and
  failed queries.

* *Skipped or failed queries*: Each query is reported as succeeded,
  skipped, or failed after installation completes.

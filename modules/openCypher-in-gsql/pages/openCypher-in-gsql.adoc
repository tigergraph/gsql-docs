...
For users migrating from Neo4j to GSQL, it is important to understand that:

* In GSQL, queries must align with the predefined schema.
* Unlike Neo4j, GSQL does not return NULL for missing attributes—if a vertex does not contain all required attributes, it is not included in the result.

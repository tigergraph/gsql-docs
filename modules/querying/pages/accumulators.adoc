...
|
|
|Returns a new bitwise accumulator whose every bit is the `AND` result of the corresponding bits in the current bitwise accumulator and the given one, without modifying the current accumulator.

| ` \|\| (accum)`
|
|
|Returns a new bitwise accumulator whose every bit is the `OR` result of the corresponding bits in the current bitwise accumulator and the given one, without modifying the current accumulator.

| ` ^ (accum)`
|
|
|Returns a new bitwise accumulator whose every bit is the Exclusive `OR` result between the current bitwise accumulator and the given one, without modifying the current accumulator.

| ` ~ (accum)`
|
|
|Returns a new bitwise accumulator in which every bit is the flipped (bitwise NOT) result of the corresponding bit in the current bitwise accumulator, without modifying the current accumulator.

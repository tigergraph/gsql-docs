= openCypher in GSQL
:description: Summary of TigerGraph's level of support for the openCypher query language.
:page-aliases: openCypher-in-gsql.adoc, openCypher-in-gsql/openCypher-in-gsql.adoc
...
TigerGraph supports the majority of openCypher features, embedded within its native GSQL language. This page lists the features currently xref:_opencypher_features_in_gsql[supported] and features xref:_opencypher_features_not_yet_supported[not yet supported ].

It also describes how the different xref:#_key_differences_gsql_vs_opencypher[schema models in OpenCypher vs. GSQL] affect some queries.
...
|CREATE |Writing create nodes and relationships. Partially supported. See the note below for limitations.
...
|REMOVE |Writing Remove properties and labels from nodes and relationships. Partially supported. See the note below for limitations.
...
|SET |Writing Update labels on nodes and properties on nodes and relationships. Partially supported. See the note below for limitations.
...
|collect() |List |Returns a list containing the values returned by an expression.
...
|endNode() |Scalar |Returns the end node of a relationship. Partially supported. See the note below for limitations.
...
|startNode() |Scalar |Returns the start node of a relationship. Partially supported. See the note below for limitations.
...
[NOTE]
====
CREATE, REMOVE, SET, startNode(), and endNode() are partially supported.
These features are supported only in single-block queries and are not supported in queries that involve tables.
====
...
[#_key_differences_gsql_vs_opencypher]

...
|Use `(source_vset) -[edge_set]- (target_vset)` notation, improving clarity and aligning with GQL style. Vertices are enclosed in parentheses () and edges in square brackets []. For example: `(Person) -[FRIENDS]- (Colleague)`, making the structure visually clearer.
...
|Supports the {m,n} syntax for quantified multi-hop patterns, e.g., `-\[FRIENDS\]->\{1,3\}`, following GQL standards.
...
* In V3, vertices are enclosed in parentheses `( )` and edges in square brackets `[ ]`, following the GQL standard for clarity and explicit representation of path patterns.
...
* In V3, quantification is expressed using `{m,n}` syntax for variable-length edge patterns. This aligns with GQL standards, providing a concise and consistent way to represent repeating edge patterns.

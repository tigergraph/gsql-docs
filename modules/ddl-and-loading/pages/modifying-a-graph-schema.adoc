...
Schema changes are essential for maintaining and improving the structure of your graph. However, they must be handled carefully to avoid disrupting ongoing operations.
...
* *Do backup your data before performing a schema change* to protect against potential data loss or corruption.
...
=== Managing Loading Jobs During Schema Changes

Currently, loading jobs during schema change jobs should be managed by *Dynamic Schema Change (DSC)*.

* If loading jobs are not affected by the schema change job, they will be managed automatically by DSC.
* If loading jobs are affected by the schema change job, the schema change job will be blocked first.
...
Specifically: 
* A loading job becomes invalid if it references a vertex or edge that has been DROPPED or ALTERED. 
* A query becomes invalid if it refers to a DROPPED vertex, edge, or attribute.

Invalid loading jobs are *DROPPED*, and invalid queries are *uninstalled*. After the schema update, users must create and install new loading and query jobs based on the updated schema.

...
For more information, see xref:tigergraph-server:manage:access/access-control-model.adoc[].
...
+
An absolute path may begin with the xref:tigergraph-server:get-started:gsql-shell/gsql-sessions.adoc[session parameter] `$sys.data_root`.
...
NOTE: This statement is not supported in a xref:data-connect:kafka-loader/index.adoc[Kafka loading job].

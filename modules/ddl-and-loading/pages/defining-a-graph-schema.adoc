...
. Using the `PRIMARY KEY` phrase with an attribute.
. Using the `PRIMARY KEY` phrase with a parenthesized list of attributes, to create a compound key out of attributes.
...
The following sections going into detail for each of the primary id/key options, attribute lists, and the vertex options.
...
By default, a vertex type which is defined with `PRIMARY_ID` cannot treat the ID as a regular vertex. For example, if the ID's field name is `serial_no`, neither of the following syntaxes are valid:

[source.wrap]
----
  v.id  //  error: id is not a built-in function or field name
  v.serial_no  //  error: not supported
----

If you want to retreive a vertex having a specific ID, you can use the `to_vertex(id, type)` function.

See the next section to see how to treat the primary ID as attribute.
...
Using this option eliminates the storage efficiency of `PRIMARY_ID` in exchange for greater flexible in query operations.
...
=== PRIMARY KEY option with a single attribute
...
[WARNING]
====
PRIMARY KEY is not supported in GraphStudio. If you define a vertex type using the PRIMARY KEY syntax, you will not be able to operate on the graph with that vertex type in GraphStudio.
====



=== PRIMARY KEY option with a composite key
...
For a compound key, the PRIMARY KEY clause should come at the end of the attribute list.
...
. There is no maximum number of attributes, but there is a maximum storage size for a xref:defining-a-graph-schema.adoc#_create_vertex[vertex].
...
The xref:defining-a-graph-schema.adoc#_primary_id_and_with_primary_id_as_attribute[primary_id_as_attribute] option was described with PRIMARY ID.

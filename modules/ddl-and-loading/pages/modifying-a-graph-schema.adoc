...
It is crucial to define and identify periods of low and high traffic in order to optimize schema changes and reduce the risk of failure.
...
* Avoid schema changes during periods of high query or data load activity. Doing so could cause failure or inconsistent behavior.
...
Schema changes will invalidate queries and loading jobs that are dependent on the schema components being modified. 
This can cause errors or failures in queries that are still trying to reference a vertex, edge, or attribute that has been altered or removed.

*What to Do:*

* After schema changes, reinstall and redefine queries and loading jobs based on the new schema.
* Make sure that the updated queries and jobs are aligned with the modified schema and are properly tested to ensure they work as expected after the changes.

[IMPORTANT]
====
Schema changes should be performed only when there are no active queries or loading jobs to avoid issues with data consistency and application errors. 
It is essential to abort all ongoing jobs before performing the schema change to prevent conflicts.

In addition, schema changes should be done during low traffic periods. High traffic periods increase the likelihood of schema change failures due to conflicts with running operations.
====

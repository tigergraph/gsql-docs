...
Use the `-zstd` option with `RUN QUERY` to request a zstd-compressed response. Use the `-o` option to specify the output file path.

[source,gsql]
----
RUN QUERY <queryName> -zstd -o "/path/to/out.zst"
----

NOTE: This option only works for installed queries.

The default zstd compression level is `3`. To change the compression level, set the `ZSTD_COMPRESSION_LEVEL` environment variable in `RESTPP.BasicConfig.Env`. For example:

...
a|`-COST`
|Installs and optimizes the query using the xref:query-optimizer/index.adoc#_cost_based_optimization[cost-based query optimizer].
The optimizer uses various statistics to generate an efficient traversal plan when compiling a query.
You can benefit from this option if you query uses a `FROM` clause that uses a xref:select-statement/index.adoc#_path_pattern[path pattern].

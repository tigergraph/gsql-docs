...
+
Only the xref:tigergraph-server:user-access:access-control-model.adoc#_query_owner[query owner] is allowed to run `CREATE OR REPLACE` on a query.
However, if you have the `WRITE_QUERY` RBAC privilege on the corresponding graph, you can execute `DROP QUERY` to drop the query and create a new one with the same name.
+
[NOTE]
====
...
* The ACLs on the original query are dropped when you drop the query.
`CREATE OR REPLACE` keeps the original ACLs intact.
====
`DISTRIBUTED`::
...
This statement type is part of either the top-level block or a query-body control flow block.For example, each of the statements at the top level directly under `CREATE QUERY` is a query-body statement.
If one of the statements is a `CASE` statement with several `THEN` blocks, each of the statements in the `THEN` blocks is also a query-body statement.Each query-body statement ends with a semicolon.
...
=== Query optimizer (preview)
When installing a query, you have the option of using the query optimizer.
The query optimizer can potentially reduce the amount of memory a query requires and the time it takes to finish execution.
The optimizer works by automatically finds the most efficient traversal plan when compiling the query during query installation.
You can benefit from this option if you query uses a `FROM` clause that performs any number of hops.

To use the query optimizer, use the `-COST` option when installing a query.
This option can be used together with other options, but it must come after the other options.
For example, you cannot run `INSTALL QUERY ALL -COST -FORCE`, it must be `INSTALL QUERY ALL -FORCE -COST`
...
a|`-COST`
|Installs and optimizes the query using the query optimizer.
This option automatically finds the most efficient traversal plan for executing the query and can reduce the time and memory it takes to execute a query.
You can benefit from this option if you query uses a `FROM` clause that performs any number of hops.
...
|`VERTEX<type>` |Since the type is already specified in the query, you only need to use a JSON object containing the field

...
| `graph`
| no
| If the graph is not specified and the current user has global proxygroup read privilege, they can see all proxy groups. Otherwise, no proxy group information can be accessed.

If the graph is specified and the current user has local proxygroup read privilege for this graph, they can only view the proxy groups on this specific graph.
|===

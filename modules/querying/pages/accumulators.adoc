...
Starting from version 4.1.2, operations between different GroupByAccums with the same data types but different field names will result in a Type Check Error during `CREATE QUERY`. In earlier versions, such queries may pass semantic checks but will fail with a compilation error during query installation.

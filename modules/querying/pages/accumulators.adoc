...
Declare bitwise accumulators as regular fixed-length (up to 64 bits), long fixed-length (over 64 bits), or dynamic-length (new in versions 4.2.1 and 4.3.0) based on your query’s needs.
...
* *Dynamic-Length*: Set the length at runtime using query parameters or variables (new in versions 4.2.1 and 4.3.0). Use .clear() to free memory. You cannot change the length after declaration (e.g., .resize() is not supported).
+
[,gsql]
----

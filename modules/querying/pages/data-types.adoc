...
The first seven types (`INT`, `UINT`, `FLOAT`, `DOUBLE`, `BOOL`, `STRING`, and `DATETIME`) are the same ones mentioned in the "xref:ddl-and-loading:attribute-data-types.adoc#_primitive_types[Attribute Data Types]" section of xref:ddl-and-loading:attribute-data-types.adoc[GSQL Language Reference, Part 1].
...
| `FLOAT`
| `0`
a| A decimal precise up to about 7 digits: `3.14159`

*NOTE*: For large numbers, `DOUBLE` is better.
...
[CAUTION]
====
Quoted values are not parsed as valid DATETIME parameters. The query may execute successfully, but DATETIME-based comparisons can return incorrect results, such as returning no matching records.

*Incorrect:*
[source,console]
----
curl -X GET 'http://127.0.0.1:9000/query/gsql_demo/dt?dt="2022-12-31T00:00:00"'
curl -X GET "http://127.0.0.1:9000/query/gsql_demo/dt?dt='2022-12-31T00:00:00'"
----
...
----
curl -X GET 'http://127.0.0.1:9000/query/gsql_demo/dt?dt=2022-12-31T00:00:00'
curl -X GET 'http://127.0.0.1:9000/query/gsql_demo/dt?dt=2022-12-31%2000%3A00%3A00'
----
====
...
[WARNING]
.Support for fixed and exponential notation
====
`FLOAT` and `DOUBLE` input values must be in fixed point `d.dddd` format, where `d` is a digit. Output values will be printed in either fixed point or exponential notation, whichever is more compact.
...
you can also authenticate using AWS role assumption. Set the `s3_assume_role` session parameter to the role ARN [4.2.3].
...
* As of 4.2, `LIST` and `MAP` of base type elements are also supported.  The key and value of a `MAP` may be xref:ddl-and-loading:attribute-data-types.adoc#_primitive_types[primitiveType] (numerical, string, or Boolean).

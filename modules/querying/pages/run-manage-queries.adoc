...
pass values to queries using GSQL and RESTPP for Graph Entity_Resolution
...
query parameter types in GSQL and through RESTPP for Graph Entity_Resolution.
...
-d '{"bag_param": [1, 2, 3, 2]}' \
"http://localhost:14240/restpp/query/Entity_Resolution/testBAG"
----
...
curl -X POST -H "Content-Type: application/json" \
-d '{"set_param": [1, 2, 3, 4]}' \
"http://localhost:14240/restpp/query/Entity_Resolution/testSET"
----
...
[source,gsql]
----
CREATE QUERY getEntityByVertex(VERTEX<Entity> vertex_param)
...
[source,bash]
----
curl -X GET "http://localhost:14240/restpp/query/Entity_Resolution/getEntityByVertex?vertex_param=Entity_1"
----
|===

NOTE: `BAG` and `SET` use the same data syntax when passed as query
parameters. A `BAG` can contain duplicate values, while a `SET` stores
only unique values.

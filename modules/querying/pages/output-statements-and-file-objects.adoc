...
If the query response is large, it is recommended to use `PRINT … TO_CSV`.
TigerGraph currently does not support pagination; returning a large query response in a single response via the GSQL terminal or RESTPP may lead to excessive memory usage and make the response difficult to parse efficiently.
====

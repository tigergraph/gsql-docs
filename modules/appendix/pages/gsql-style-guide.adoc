...
<2> It is acceptable to combine SELECT and FROM on the same line, if the combined line meets the line length requirement.
Avoid such combination for any other two query keywords.
...
In GSQL, SELECT statements begin with a vertex set variable. 
This means SELECT is not in the leftmost column. If you prefer, you can break the first clause (`<vertex set variable> = SELECT <vertex alias>`) into two lines, putting an indented SELECT on the second line:

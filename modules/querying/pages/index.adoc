...
A typical workflow for working with GSQL queries is to xref:create-and-install-queries.adoc#_create_query[create the query], xref:create-and-install-queries.adoc#_install_query[install the query], and then xref:run-manage-queries.adoc#_run_query[run the query].

GSQL also supports xref:run-manage-queries.adoc#interpret-a-created-query[interpreting a query], which allows users to skip query installation and execute the query directly.
...
* xref:run-manage-queries.adoc[Run and Manage Queries]
* xref:data-types.adoc[]

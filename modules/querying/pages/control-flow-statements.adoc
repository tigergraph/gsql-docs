...
You may also be interested in xref:operators-and-expressions.adoc#_case_expressions[CASE expressions].
...
For more detailed general examples of the difference between query-body statements and DML-sub statements, see xref:create-and-install-queries.adoc#_statement_types[statement types].
...
=== CASE Block Syntax
The punctuation used for statements within a CASE `THEN` or `ELSE` block depends on where the CASE statement is used.

At the query-body level, statements within each `THEN` or `ELSE` block are query-body statements and are separated by semicolons. The CASE statement itself also ends with a semicolon.
...
At the DML-sub level, statements within each `THEN` or `ELSE` block are DML-sub statements and are separated by commas. There is no punctuation after the final statement before `END`.

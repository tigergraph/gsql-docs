...
|Use `source_vset -(edge_set)- target_vset` notation.
|Use `(source_vset) -[edge_set]- (target_vset)` notation, improving clarity and aligning with GQL style.

|Alias Order
|Object sets and aliases follow `source_vset:S -(edge_set:E)- target_set:T`.
|Object sets and aliases follow `(S:source_vset) -[E:edge_set]- (T:target_set)`, making alias assignment explicit and intuitive.

|Symbols for Edge Direction
|Arrowheads `<` or `>` are placed inside parentheses after the edge type to indicate direction (e.g., `-(EdgeType:e)-`).
|Same as V2, but notation uses square brackets for edges, such as `[edge_set]` for improved clarity.

|Embedded Edge Filtering
|Filtering conditions for edges are applied outside the `FROM` clause using `WHERE`.
|Filtering conditions for edges can now be embedded directly in the `FROM` clause for concise filtering. For example: `(Person) -[Friend WHERE Friend.length > 2]-> (Colleague)`.

|Path Patterns and Multi-Hops
|Multi-hop path patterns are supported with `-(edge_set)-`.
|Supports multi-hop path patterns using `(source_vset) -[edge_set]- (target_vset)`. Updated notation provides better readability and clarity.

...
Token functions are functions in the DDL language that operate on tokens.
They can be used to construct attribute expressions or for conditional expressions in the `WHERE` clause.

To use a token function, replace the attribute in the destination clause of the `LOAD` statement with the function call.
The arguments of the function can be a column from the `FILE` object.
...
For a full list of token functions, see xref:functions/token/index.adoc[].

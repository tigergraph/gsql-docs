...
'''
=== get schema information from a Snowflake data source


`GET /gsql/v1/metadata/data-sources/{dsName}`

This endpoint is used to get schema information from a Snowflake data source.
`{dsPath}` is the name of a previously created Snowflake data source object.
...
[source,bash]
----
curl -X GET 'Content-type: text/plain' "http://localhost:14240/gsql/v1/metadata/data-sources/k1"
----
--
Sample Response::
+

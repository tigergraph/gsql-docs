...
In classic (Syntax v1) GSQL, the FROM clause described one step or hop pattern. In xref:tutorials:pattern-matching/[Pattern Matching] (Syntax v2), the pattern can be multiple hops long. Path patterns also have many other options for finer control and greater flexibility.

...
* When a bitwise accumulator length is longer than 64 bits, it's assigned using an array of integers, one integer for each group of 64 bits, e.g., [int3, int2, int1].
Each integer is converted to a 64-bit sequence of 1s and 0s.
The rightmost (last) element in the array is for the 64 least significant bits. If the total number of bits is not a multiple of 64, then the leftmost (first) element in the array will be for fewer than 64 bits.
* The previous documentation incorrectly suggested a maximum length of 2 x 64 bits (128 bits) for bitwise accumulators. There is no length limit in the latest version, though bugs affecting functions longer than 64 bits have been fixed in version 4.2.0.
* Long bitwise accumulators can be initialized using a list of 64-bit integer values:[value1, value2, value3, etc.]
...
|
|
|Compares two bitwise accumulators of the same length and returns a new bitwise accumulator whose every bit is the `AND` result of the corresponding bits in the current bitwise accumulator and the given one, without modifying the current accumulator.

| `.\|\| (accum)`
|
|
|Compares two bitwise accumulators of the same length and returns a new bitwise accumulator whose every bit is the `OR` result of the corresponding bits in the current bitwise accumulator and the given one, without modifying the current accumulator.

| `.^ (accum)`
|
|
|Compares two bitwise accumulators of the same length and returns a new bitwise accumulator whose every bit is the `XOR` result of the corresponding bits in the current bitwise accumulator and the given one, without modifying the current accumulator.

| `.~ (accum)`
|
|
|Returns a new bitwise accumulator in which every bit is flipped (bitwise NOT) from the corresponding bit in the current bitwise accumulator, without modifying the current accumulator.
...
**Comparator Support:**Bitwise accumulators now support comparators including <, >, ==, !=, < =, and >=.

**Example:** WHERE (@@BitwiseAccum1 > 32) AND (@@BitwiseAccum2 < 1024)

**Implementation Differences**

**Standard Bitwise Accum:** Implicitly convert it to an integer for comparison.

**User-defined Bitwise Accum:** Comparators are overridden for bitwise accumulators longer than 64 bits.

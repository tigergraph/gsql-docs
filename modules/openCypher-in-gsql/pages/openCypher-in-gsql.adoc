...
== Key Differences: GSQL OpenCypher mode vs. OpenCypher

GSQL OpenCypher is strongly typed and **schema-first**, while OpenCypher is **schema-optional**. This fundamental difference impacts how OpenCypher queries behave in each system.
...
==== How GSQL OpenCypher Interprets This Query
...
* In GSQL OpenCypher, queries must align with the predefined schema.
* Unlike OpenCypher, GSQL OpenCypher does not return NULL for missing attributes—if a vertex does not contain all required attributes, it is not included in the result.
* A query attempting to access a non-existent attribute in GSQL OpenCypher may either return no results or be rejected if no matching vertex type exists.

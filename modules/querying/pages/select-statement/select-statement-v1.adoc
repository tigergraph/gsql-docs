...
* In *V2* and *V3* syntax, only left-to-right directed edges are supported. Right-to-left directed edges (e.g., `FROM start:s \<-(:e)- :v`) are not supported in either V2 or V3 syntax.
* The `SAMPLE` clause does not guarantee returning the exact number of elements; the number of samples returned may vary.
====

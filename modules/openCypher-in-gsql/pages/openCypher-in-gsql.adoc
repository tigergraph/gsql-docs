...
[NOTE]
====
From version *4.1.2*, certain openCypher syntaxes are now fully supported, eliminating previous restrictions.
====
...
These were limitations before *4.1.2* – certain openCypher syntaxes were not supported:
...
== Key Differences: GSQL vs. OpenCypher

GSQL is strongly typed and **schema-first**, while OpenCypher is **schema-optional**. This fundamental difference impacts how OpenCypher queries behave in each system.

...
To learn more about permission and privileges, see xref:tigergraph-server:manage:access/access-control-model.adoc[].
====
...
For more about roles and privileges, see the document xref:tigergraph-server:manage:access/index.adoc[Managing User Privileges and Authentication].
...
Unlike xref:get-started:database-definition.adoc#_reset_all[the `gsql --reset` command], `DROP ALL` does not erase user and role information.

...
runOrInterpretNamedQuery := (RUN | INTERPRET) QUERY [runOptions] queryName [modeOption] "(" parameterValueList | parameterValueJSON ")"
modeOption := "-mode" ( "legacy" | "instruction" | "debug" )
...
[source,ebnf]
----
runOrInterpretNamedQuery := (RUN | INTERPRET) QUERY [runOptions] queryName [modeOption] "(" parameterValueList ")"
modeOption := "-mode" ( "legacy" | "instruction" | "debug" )
----
...
Starting in TigerGraph 4.3.0, you can control the interpret engine with the `-mode` option:

* `-mode legacy`: Forces the legacy engine (supports all GSQL features, slower)
* `-mode instruction`: Forces the instruction engine (faster, fails if query uses unsupported features).
* `-mode debug`: Forces instruction mode and generates debug files (`serialized_instructions.json` and `serialized_instructions.bin`) in `/tmp/gpe` under the TigerGraph installation folder. Use this for advanced troubleshooting, typically by developers.

If you don’t specify a `-mode`, the system automatically picks the best mode, preferring instruction mode.

.Example of Interpret with Modes

[source,gsql]
----
INTERPRET QUERY myQuery("Jane.Doe")              // Auto-selects mode
INTERPRET QUERY myQuery -mode legacy("Jane.Doe") // Forces legacy mode
INTERPRET QUERY myQuery -mode instruction("Jane.Doe") // Forces instruction mode
INTERPRET QUERY myQuery -mode debug("Jane.Doe")  // Forces instruction mode with debug files
----

[#checking-interpret-modes-and-logs]
=== Checking Interpret Modes and Logs

Users can confirm which mode was used by checking GSQL logs:
...
[#semantic-errors]
=== Semantic Errors for Interpret Modes

When using the `-mode` option, you may see one of the following errors:

* `Query [query_name] uses instruction only features, but is run in legacy mode.`
+
The query uses instruction-only features but was forced into legacy mode.

* `Query [query_name] uses legacy only features, but is run in instruction mode.`
+
The query requires the legacy engine but was forced into instruction mode.

* `Query [query_name] cannot be executed in both instruction mode and legacy mode at the same time.`
+
The query mixes features from both engines. To resolve this, install the query and run it with `RUN QUERY` instead of `INTERPRET QUERY`.

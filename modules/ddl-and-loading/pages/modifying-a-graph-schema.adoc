...
[NOTE]
====
Graphs can define local vertex and edge types independently of the local or global vertex and edge types in other graphs.
That is, local vertex and edge types can share the same name across different graphs, while global vertex or edge types used by multiple graphs cannot share the same name.
====

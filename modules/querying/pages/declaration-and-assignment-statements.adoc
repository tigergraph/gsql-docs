...
fileDeclStmt := FILE fileVar "(" filePath ["," filePermissionCode] ")"
fileVar := name
filePath := stringLiteral
filePermissionCode := stringLiteral   // octal encoded permission as a string literal
...
In GSQL, curly brackets, as well as `IF .. THEN`, `ELSE`, `+WHILE ... DO+`, `+FOREACH ... DO+` statements create a block.
A `SELECT` statement also creates a block.A block-scoped variable declared inside a block scope is only accessible inside that scope.
...
A global-scoped variable is always accessible anywhere in the query once it has been declared regardless of where it is declared.
One also cannot declare another variable with the same name as a global-scoped variable that has already been declared.
...
In a GSQL query body, variables holding values of types `INT`, `UINT`, `FLOAT`, `DOUBLE`, `BOOL`, `STRING`, `DATETIME`, `VERTEX`, `EDGE`, `JSONOBJECT` and `JSONARRAY` are called base type variables.
The scope of a base type variable is from the point of declaration until the end of the block where its declaration took place.
...
|`SET`
|`INT`, `UINT`, `DOUBLE`, `FLOAT`, `STRING`, `JSONOBJECT`, `JSONARRAY`,`BOOL` `EDGE`, `DATETIME`, tuple

|`BAG`
|`INT`, `UINT`, `DOUBLE`, `FLOAT`, `STRING`, `JSONOBJECT`, `JSONARRAY`,`BOOL`, `VERTEX` `EDGE`, `DATETIME`, tuple

|`LIST`
|`INT`, `UINT`, `DOUBLE`, `FLOAT`, `STRING`, `JSONOBJECT`, `JSONARRAY`,`BOOL`, `VERTEX` `EDGE`, `DATETIME`, `ListAccum` (Up to 3 levels of nesting), tuple

|`MAP`
a|* Key types: `INT`, `UINT`, `DOUBLE`, `FLOAT`, `STRING`, `JSONOBJECT`, `JSONARRAY`,`BOOL`, `VERTEX` `EDGE`, `DATETIME`, tuple
...
==== Vertex set seeding from string values

A `STRING`, `SET<STRING>`, or `SetAccum<STRING>` can be used to initialize a vertex set when the values of the `STRING`, `SET<STRING>`, or `SetAccum<STRING>` are valid vertex type names.

The `STRING`, `SET<STRING>`, and `SetAccum<STRING>` forms behave equivalently when they contain the same vertex type names.
...
[source,gsql]
----
include::appendix:example$social_net/vertex_set_seeding_from_string_values.gsql[]
----
--
Result::
+
--
.vertex_set_seeding_from_string_values Query Results
[source,gsql]
----
GSQL > RUN QUERY vertex_set_seeding_from_string_values("Account_1", ["Account_1"])
include::appendix:example$social_net/vertex_set_seeding_from_string_values_results.json[]
----
--
====

If any supplied string is not a valid vertex type name, GSQL reports a runtime error.

[source,text]
----
GSQL > RUN QUERY vertex_set_seeding_from_string_values(
    "NotARealVertexType",
    ["NotARealVertexType"]
)

Runtime Error: 'NotARealVertexType' is not valid vertex type.
----
...
Because the type of vertex parameter `m1` is `person`, the GSQL engine will implicitly assign `S` to be `person` type, making it unnecessary to explicitly define the vertex type of `S`.
...
A `FILE` object is a sequential text storage object, associated with a text file on the local machine or with a cloud storage bucket.
...
The path should start with `s3://`, followed by the bucket name and the S3 path, e.g., `s3://bucket-name/queryoutput/output.csv`. During the execution of the query, content will be uploaded to the S3 bucket. Append mode writing is not supported.
If an S3 object with the same path already exists, it will be overwritten.

For S3 access credentials, file path conflict handling, and connection error codes, see xref:write-output-to-cloud.adoc[Write Output to Cloud].

...
=== Loading a nullable attribute [v4.3+]

`NULL` values can be loaded into vertex or edge attributes
...
`NULL` values are also supported when xref:ddl-and-loading:running-a-loading-job.adoc[Running a Loading Job]

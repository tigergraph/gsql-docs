...
For a detailed description, please refer to xref:{page-component-version}@tigergraph-server:user-access:fine-grained-query-privileges.adoc[Fine-Grained Query Privileges].
====
...
[CAUTION]
====
Installing a query takes several seconds. Query installation and query execution cannot run concurrently on the same graph. Query execution is delayed until query installation finishes.

Only one `INSTALL QUERY` command can run on a given graph at a time. Concurrent `INSTALL QUERY` commands are allowed when they target different graphs.
====

...
Jobs and queries for unaltered parts of the schema will still be available and do not need to be reinstalled.
However, even though these jobs are valid (e.g., they *can* be run), consider whether they will still perform the desired operations after the change (e.g., do you *want* to run them?).

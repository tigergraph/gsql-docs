...
The name of a vertex-attached or a edge-attached accumulator begins with a single `@`.
...
[NOTE]
====
Edge-attached Accumulators can only be used in a distributed and interpret queries.
====
...
Additionally, the operator `+=` can be used to update the state of the accumulator; the function of `+=` depends on the accumulator type.
Edge-attached accumulators can only be accessed or updated (via `=` or `+=`) in an `ACCUM/WHERE` clauses within a `SELECT` statement.

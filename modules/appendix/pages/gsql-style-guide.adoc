...
This means SELECT is not in the leftmost column. If you prefer, you can break the first clause (`<vertex set variable = SELECT`) into two lines, putting an indented SELECT on the second line:

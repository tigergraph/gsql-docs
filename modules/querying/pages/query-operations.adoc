...
See the
xref:querying:index.adoc#_gsql_syntax_versions[GSQL syntax versions] for an outline of the differences.
See
...
Query Mode].
| `-legacy`
a| Install the query in legacy mode.
The query will compile using the legacy compilation mode.

The only user-facing different between the current compilation mode and the legacy mode is that the loop variable in a `xref:querying:control-flow-statements.adoc#_foreach_statement[FOREACH` loop] is treated as a copy in the current compilation mode, while in legacy mode, changes to the loop variable will change the value in the set or bag expression. 
|===
...
Queries executed in Detached mode are still subject to the system timeout limit. The default timeout limit is 16 seconds and can be set using the xref:tigergraph-server:API:intro.adoc#_gsql_query_timeout[`GSQL-TIMEOUT`] header.

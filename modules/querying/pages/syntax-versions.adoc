...
|Alias Ordering
|Object sets and aliases follow `source_vset:Alias -(edge_set:EdgeAlias)- target_set:Alias`.
|Object sets and aliases follow `(Alias:source_vset) -[EdgeAlias:edge_set]- (Alias:target_set)`, reversing the order of alias vs. object set.

|Symbols for Edge Direction
|Directionality is applied at the edge-set level. Supports `<` (left), `>` (right), and `_` (undirected). For example: `-(<Edge>:Alias)-`.
|Directionality applies to the entire edge pattern. Symbols include `<-` (left), `->` (right), `~` (undirected). Example: `<-[Alias:Edge]->`.

|Edge Direction Syntax
|Supports granular directionality at the individual edge set level. Multiple edge sets in a pattern can have distinct directions.
|Directionality applies globally at the edge pattern level, making the syntax more concise.

|Quantified Multi-Hop Patterns
|Use explicit repetition for multi-hop patterns, e.g., `-(FRIENDS*1..3)->`.
|Uses {m,n} GQL-style quantifiers for conciseness, e.g., `-[FRIENDS]->{1,3}`.
...
* Concise syntax for expressing variable-length edge patterns.
...
* In V3, the `~[A:U1]~` syntax is used for the same purpose, aligning with modern GQL standards.

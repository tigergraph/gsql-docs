...
The xref:tigergraph-server:api:gsql-endpoints.adoc#_run_a_loading_job[`maxPercentError`] key for running a loading job via the REST API has the same meaning as the `-max_percent_error` option in GSQL.
...
The xref:tigergraph-server:api:gsql-endpoints.adoc#_run_a_loading_job[`maxNumError`] key for running a loading job via the REST API has the same meaning as the `-max_error` option in GSQL.

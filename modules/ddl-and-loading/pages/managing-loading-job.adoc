...
== Failure Outcomes for Streaming and Batch (Bulk) Loading Jobs
=== Batch (File) Loading Jobs
...
* Neither streaming nor batch loading jobs support job-level transactional atomicity.
* If a job fails and cannot resume, already loaded data remains in the graph and cannot be reverted.
* Unlike databases that use temporary tables for atomicity, TigerGraph does not currently support this functionality.

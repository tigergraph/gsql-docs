...
a| 4-byte single-precision floating point number  Examples: 3.14159, .0065e14, 7E23.  *See notes below*.
...
Has the same input and output format as FLOAT, but the range and precision are greater. *See notes below*.
...
.FLOAT and DOUBLE
====
Remember that FLOAT and DOUBLE have limited precision.
In particular, for large numbers that require precision, `DOUBLE` is the better choice.
====
...
[NOTE]
====
All the data types listed above, when used as attributes of a vertex or edge, can be set to `NULL`.
====
...
[NOTE]
====
`LIST`, `SET`, `MAP` can be assigned `NULL` as an attribute. For `LIST` and `SET`, a `NULL` attribute is not the same as `[NULL, NULL]` or `{NULL}`, and `NULL` elements are not allowed inside the collection. For `MAP`, neither keys nor values can be `NULL`.
====

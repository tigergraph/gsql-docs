...
=== Adding floating point values at different scales

Imagine you have two `FLOAT` variables:
...
But since `FLOAT` offers only about 7 digits of precision, it is impossible to adequately capture B's contribution.
This problem can occur inadvertently when adding a large set of numbers.
The individual numbers may be about the same scale, but the intermediate sums are of a larger sum.

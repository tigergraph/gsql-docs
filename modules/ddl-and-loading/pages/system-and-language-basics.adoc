...
|`cost-opt`| Whether to use the xref:querying:query-optimizer/index.adoc[query optimizer] when  installing a query. If set to true, query installation during this session uses the optimizer to optimize the query based on cost.
|===
